...
import { X, Ruler, XCircle, Check } from 'lucide-react'; // Pridané ikony pre stavy
import { cn } from '@/lib/utils';

// Typ pre veľkosť
interface AvailableSize { size: string; stock: number; id?: string | null; }
...
const SizeSelectorDrawer: React.FC<SizeSelectorDrawerProps> = ({
    isOpen, onOpenChange, availableSizes, selectedSize, onConfirm,
}) => {
    const [tempSelectedSize, setTempSelectedSize] = useState<string | null>(selectedSize);
    ...
    const handleSizeClick = (size: string) => { setTempSelectedSize(size); };
        ...
        <Sheet open={isOpen} onOpenChange={onOpenChange}>
            ...
            >
                ...
                <SheetHeader className="px-4 sm:px-6 py-4 border-b border-border flex flex-row justify-between items-center">
                    ...
                    <SheetClose asChild>
                        <Button variant="ghost" size="icon" className="h-8 w-8 rounded-full -mr-2"> {/* Menšie close tlačidlo */}
                ...
                {/* === Obsah s Veľkosťami === */}
                <ScrollArea className="flex-grow px-4 sm:px-6 py-5"> {/* Vertikálny padding */}
                    {/* Mriežka veľkostí */}
                        ...
                        {availableSizes.map(({ size, stock }) => {
                            ...
                            return (
                                ...
                                >
                                    ...
                                    {isAvailable && stock <= 5 && ( // Zobraz len pri nízkom sklade
                                    ...
                                    {!isAvailable && (
                                        <div className="absolute inset-0 flex items-center justify-center bg-background/50 dark:bg-black/50 backdrop-blur-sm rounded-md">
                                            <XCircle className="w-5 h-5 text-muted-foreground/70" strokeWidth={1.5}/>
                                        </div>
                                    )}
                         ...
                         {availableSizes.length === 0 && (
                             <p className="text-sm text-muted-foreground italic col-span-full py-4 text-center"> {/* col-span-full pre zabratie šírky */}
                 ...
                 <div className="px-4 sm:px-6 py-4 border-t border-border">
                    ...
                    >
                         <Ruler size={18} className="mr-2.5 text-muted-foreground" /> {/* Ikona s odsadením */}
                         Veľkostná tabuľka
                     </Button>
                 </div>
                ...
                <SheetFooter className="px-4 sm:px-6 py-4 border-t border-border bg-muted/30 dark:bg-card/50 flex flex-row sm:justify-between items-center gap-2 mt-auto">
                     {/* Indikátor výberu */}
                     <div className="text-sm text-muted-foreground flex-shrink-0 hidden xs:block"> {/* Skryjeme na veľmi malých */}
                         Výber: <span className="font-semibold text-foreground">{tempSelectedSize || '-'}</span>
                     </div>
                     {/* Akčné tlačidlá */}
                     <div className="flex gap-2 flex-grow justify-end">
                        <SheetClose asChild>
                            {/* Použijeme menší variant pre "Zatvoriť" */}
                            <Button variant="outline" size="lg" className="h-11 px-5">Zatvoriť</Button>
                        </SheetClose>
                        <Button
                            size="lg" // Väčšie tlačidlo
                            onClick={handleConfirmClick}
                            disabled={!tempSelectedSize || availableSizes.find(s => s.size === tempSelectedSize)?.stock <= 0}
