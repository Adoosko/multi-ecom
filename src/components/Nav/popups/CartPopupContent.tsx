...
import { Separator } from '@/components/ui/separator'; // Použijeme len ako alternatívu k divide-y
import QuantitySelector from '@/components/ui/quantity-selector'; // Náš selector
import { Trash2, ShoppingCart, Loader2 } from 'lucide-react'; // Ikony
...
const CartItemDisplay: React.FC<{ item: CartItem }> = ({ item }) => {
    ...
    return (
      ...
      <div className="flex items-start gap-4 py-4"> {/* Väčší padding */}
        {/* Obrázok */}
        <div className="relative h-20 w-20 flex-shrink-0 overflow-hidden rounded-md border border-border bg-muted/50"> {/* Väčší obrázok */}
          {item.image ? (
            <Image
              src={item.image}
              alt={item.title}
            ...
            <div className="flex h-full w-full items-center justify-center bg-secondary">
              <ShoppingCart size={32} className="text-muted-foreground" /> {/* Väčšia ikona */}
            </div>
          )}
        </div>
  ...
  return (
    <div className={cn(
      ...
      <div className="px-4 sm:px-6 py-4 border-b border-border flex justify-between items-center flex-shrink-0">
        <h4 className="font-semibold text-base text-foreground">Nákupný košík</h4>
      ...
      {itemCount > 0 ? (
        <>
          {/* Zoznam položiek */}
          <ScrollArea className="flex-grow px-4 sm:px-6"> {/* Odsadenie obsahu */}
            ...
            {items.length > itemsToShow.length && (
              <p className="text-xs text-muted-foreground text-center pt-3 pb-1">
                A ďalších {items.length - itemsToShow.length} {items.length - itemsToShow.length === 1 ? 'položka' : 'položiek'}...
              </p>
            )}
          </ScrollArea>

          {/* Pätička */}
          <div className="px-4 sm:px-6 py-4 mt-auto border-t border-border space-y-4 flex-shrink-0 bg-popover"> {/* bg-popover pre istotu prekrytia */}
             <div className="flex items-center justify-between text-base">
                <span className="font-medium text-muted-foreground">Celkom:</span>
                <span className="font-bold text-lg text-foreground">{formatPrice(cartTotal)}</span>
             </div>
             <Link href="/kosik" onClick={onLinkClick} legacyBehavior>
                <Button size="lg" className="w-full bg-primary hover:bg-primary/90 text-primary-foreground dark:bg-red-700 dark:hover:bg-red-800 dark:text-white h-11 text-base">
                  Zobraziť košík
                </Button>
             </Link>
          </div>
        ...
        <div className="flex-grow flex flex-col items-center justify-center px-6 py-10 text-center">
            ...
            <p className="text-muted-foreground">Váš košík je prázdny.</p>
             <Link href="/produkty" onClick={onLinkClick} legacyBehavior>
                 <Button variant="outline" size="sm" className="mt-6">Pokračovať v nákupe</Button>
             </Link>
        </div>
      )}
    </div>
