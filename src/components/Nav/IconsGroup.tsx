...
import { Sheet, SheetContent, SheetTrigger } from "@/components/ui/sheet"; // Import shadcn Sheet
...
const IconsGroup: React.FC<IconsGroupProps> = ({ isUserLoggedIn = false }) => {
  ...
  const renderUserTrigger = (isSheetTrigger = false) => (
    <button
      ...
      onClick={
        !isSheetTrigger && !isDesktop
          ? () => {
              /* No action, Link handles nav */
            }
          : undefined
      }
  ...
  const renderCartTrigger = (isSheetTrigger = false) => <CartIcon />;
    ...
    <div className="flex items-center space-x-1 md:space-x-2">
      ...
      {isDesktop ? (
        <div
          ...
          onMouseLeave={() =>
            closePopupWithDelay(setIsUserPopupOpen, userTimeoutRef)
          }
        >
          {/* Link pre navigáciu, aj keď je popup */}
          <Link
            href={isUserLoggedIn ? "/ucet" : "/prihlasenie"}
            legacyBehavior
            passHref
          >
          ...
          <div
            style={{
              pointerEvents: "none",
            }} /* Popup nemá zachytávať pointre, len sa zobraziť */
          >
            <div
              onMouseEnter={() => cancelClose(userTimeoutRef)}
              onMouseLeave={() =>
                closePopupWithDelay(setIsUserPopupOpen, userTimeoutRef)
              }
            >
        ...
        <Sheet open={isUserSheetOpen} onOpenChange={setIsUserSheetOpen}>
          <SheetTrigger asChild>{renderUserTrigger(true)}</SheetTrigger>
          <SheetContent
            side="right"
            className="bg-neutral-900 border-l-neutral-700 text-white w-[300px] sm:w-[400px] p-0"
          >
            {/* Obsah sa renderuje tu */}
            <UserProfileContent
              isUserLoggedIn={isUserLoggedIn}
              onLinkClick={handleCloseUserSheet}
            />
          </SheetContent>
        </Sheet>
      )}
      ...
      {isDesktop ? (
        <div
          ...
          onMouseLeave={() =>
            closePopupWithDelay(setIsCartPopupOpen, cartTimeoutRef)
          }
        >
          ...
          <div style={{ pointerEvents: "none" }}>
            <div
              onMouseEnter={() => cancelClose(cartTimeoutRef)}
              onMouseLeave={() =>
                closePopupWithDelay(setIsCartPopupOpen, cartTimeoutRef)
              }
            >
        ...
        <Sheet open={isCartSheetOpen} onOpenChange={setIsCartSheetOpen}>
          <SheetTrigger asChild>{renderCartTrigger(true)}</SheetTrigger>
          <SheetContent
            side="right"
            className="bg-neutral-900 border-l-neutral-700 text-white w-[300px] sm:w-[400px] p-0"
          >
