...
const MobileNavItem: React.FC<MobileNavItemProps> = ({
  ...
  if (item.megaMenu && item.megaMenu.length > 0) {
    return (
      ...
      >
        <CollapsibleTrigger
          className={`flex justify-between items-center w-full px-3 py-3 rounded-md text-base font-medium ${theme === "dark" ? "text-gray-200 hover:bg-neutral-800 hover:text-white" : "text-gray-900 hover:bg-gray-100 hover:text-gray-900"} transition-colors duration-150 ${theme === "dark" ? "data-[state=open]:bg-neutral-800 data-[state=open]:text-white" : "data-[state=open]:bg-gray-100 data-[state=open]:text-gray-900"}`}
        >
          <span>{item.name}</span>
          <ChevronDown
            className={`h-5 w-5 ${theme === "dark" ? "text-gray-400" : "text-gray-500"} group-data-[state=open]/collapsible:rotate-180 transition-transform duration-200`}
          />
        </CollapsibleTrigger>
        <CollapsibleContent
          className={`pl-6 pr-3 pb-2 space-y-1 ${theme === "dark" ? "bg-gray-750" : "bg-gray-50"} rounded-b-md overflow-hidden data-[state=closed]:animate-collapsible-up data-[state=open]:animate-collapsible-down`}
        >
