...
import type { Product, Category } from '@/payload-types'; // Payload typy


import { Breadcrumbs } from '@/components/ui/breadcrumbs'; // Breadcrumbs
import CategoryClientPage from './CategoriesClientPage';

// Typy pre parametre stránky
...
export async function generateMetadata({ params }: CategoryPageParams): Promise<Metadata> {
  ...
  try {
    ...
    const metadataDescription = category.slug ?? `Prehľad produktov v kategórii ${category.name}`;
    // TODO: Pridať obrázok kategórie pre OG/Twitter, ak existuje

    return {
      title: `${metadataTitle} | ${siteName}`,
      description: typeof metadataDescription === 'string' ? metadataDescription : '', // RichText by potreboval konverziu na string
      openGraph: {
        title: metadataTitle,
        description: typeof metadataDescription === 'string' ? metadataDescription : '',
        type: 'website',
...
// --- Hlavný Server Component Stránky ---
export default async function CategoryPage({ params, searchParams }: CategoryPageParams) {
  ...
  const page = parseInt(searchParams.page || '1', 10); // Získaj číslo stránky z URL, default 1
  ...
  try {
    ...
    productsResult = { // Vytvoríme fallback štruktúru
        docs: [],
        totalDocs: 0,
        limit: limit,
        totalPages: 0,
  ...
  const breadcrumbItems = [
    ...
    { label: category.name ?? 'Kategória', href: `/kategorie/${category.slug}`, isActive: true },
  ];
