...
const ProductClientPage: React.FC<ProductClientPageProps> = ({ product }) => {
  // --- Správa Stavov ---
  const [selectedColorVariant, setSelectedColorVariant] =
    useState<ClientColorVariant | null>(null);
  const [selectedSizeVariant, setSelectedSizeVariant] =
    useState<SelectedSize | null>(null);
  ...
  const colorVariants = useMemo((): ClientColorVariant[] => {
    ...
    return product.colorVariants.map((cv) => {
      const colorImage =
        typeof cv.image === "object" && cv.image !== null
          ? (cv.image as Media)
          : null;
      return {
        id: cv.id || null,
        ...
        image: colorImage
          ? { url: colorImage.url!, alt: colorImage.alt }
          : null,
        sizes: (cv.sizes || [])
          .map((s) => ({
            id: s.id || null,
  ...
  const galleryImages = useMemo(() => {
    ...
    const addImage = (
      imgData: Media | null | undefined,
      defaultAlt: string
    ) => {
      const img =
        typeof imgData === "object" && imgData !== null ? imgData : null;
    ...
    colorVariants.forEach((cv) =>
      addImage(cv.image as Media | null, `${product.title} - ${cv.color}`)
    );
    (product.images || []).forEach((imgItem) =>
      addImage(
        imgItem.image as Media | null,
        imgItem.alt || product.title || "Produkt"
      )
    );
  ...
  useEffect(() => {
    ...
    if (selectedColorVariant) {
      const firstAvailableSize = selectedColorVariant.sizes.find(
        (s) => s.stock > 0
      );
      setSelectedSizeVariant(firstAvailableSize || null); // Nastav prvú dostupnú alebo null
      setMainImage(
        selectedColorVariant.image?.url ||
          galleryImages[0]?.url ||
          "/placeholder.png"
      );
    } else {
  ...
  const handleColorChange = (colorName: string | null) => {
    const newColorVariant =
      colorVariants.find((cv) => cv.color === colorName) || null;
    if (newColorVariant !== selectedColorVariant) {
      // Zmena len ak je iná farba
  ...
  const handleSizeChange = (sizeName: string | null) => {
    const newSizeVariant =
      availableSizes.find((s) => s.size === sizeName) || null;
    if (newSizeVariant !== selectedSizeVariant) {
      // Zmena len ak je iná veľkosť
  ...
  const handleQuantityChange = (newQuantity: number) => {
    ...
    const validatedQuantity = isNaN(newQuantity)
      ? 1
      : Math.max(1, Math.min(newQuantity, stock > 0 ? stock : 1));
    ...
    try {
      ...
      toast.success(
        ...
        </div>,
        {
          ...
          duration: 3500,
          className:
            "border border-border shadow-lg rounded-xl bg-popover text-popover-foreground",
    ...
    >
      ...
      <div className="max-w-screen-xl mx-auto px-4 pt-8 pb-16 sm:px-6 lg:px-8 lg:pt-12 lg:pb-24">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16 items-start">
          <motion.div
            variants={fadeInItem}
            className="lg:sticky lg:top-[calc(var(--header-height,64px)+1.5rem)]"
          >
            <ProductImageGallery
