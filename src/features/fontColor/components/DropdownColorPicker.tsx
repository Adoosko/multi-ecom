...
export const DropdownColorPicker: React.FC = () => {
  ...
  const applyStyleTextToNodes = useCallback(
  ...
  const setPickerColorFromSelection = useCallback(() => {
    editor.getEditorState().read(() => {
      // Použijeme read namiesto update, lebo len čítame
      const selection = $getSelection();
      if (!$isRangeSelection(selection)) {
      ...
      const firstCssVar: string | null = null;
  ...
  const handleFontColorChange = useCallback(
    (color: string, cssVariableColor?: string | null) => {
  ...
  const handleApplyStyles = useCallback(() => {
    if (fontColor !== null) {
      // Aplikujeme len ak je farba definovaná
  ...
  return (
    <DropdownMenu onOpenChange={handleOpenChange}>
      ...
      <DropdownMenuContent
        side="top"
        className="bg-gray-800 border-gray-700 text-white p-0"
      >
